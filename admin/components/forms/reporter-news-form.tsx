...
const iconToolbarButtonActiveClass =
...
export function ReporterNewsForm({ authorName = "Reporter", onSuccess, postToEdit = null, onCancelEdit }: ReporterNewsFormProps) {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: "Write your article here... Add headings, links, and images.",
      }),
    ],
    content: "",
    editorProps: {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        const data = (await response.json().catch(() => [])) as Array<{ name?: string }>;
        const dynamicCategories = data
          .map((item) => item.name?.trim())
          .filter((item): item is string => Boolean(item));

        if (dynamicCategories.length > 0) {
          const unique = Array.from(new Set([...categories, ...dynamicCategories]));
          setCategoryOptions(unique);
        }
      } catch {
        // Keep local defaults when category API is unavailable.
      }
    };
  ...
  useEffect(() => {
    ...
    const incomingCategory = (postToEdit.category || categories[0]).trim();
    const isKnownCategory = categoryOptions.includes(incomingCategory);
    setCategory(isKnownCategory ? incomingCategory : customCategoryValue);
    setCustomCategoryEnabled(!isKnownCategory);
    setCustomCategory(isKnownCategory ? "" : incomingCategory);
    if (!isKnownCategory && incomingCategory) {
      setCategoryOptions((previous) => Array.from(new Set([...previous, incomingCategory])));
    }
    setDescription(postToEdit.description || "");
    setTags((postToEdit.tags || []).join(", "));
    setScheduledAt(toDateTimeLocalValue(postToEdit.scheduledAt));
    setSubmitIntent(postToEdit.isPublished ? "published" : "draft");
    editor.commands.setContent(postToEdit.content || "");
    setMessage(null);
  }, [editor, postToEdit, categoryOptions]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const effectiveCategory = customCategoryEnabled ? customCategory.trim() : category;
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            {customCategoryEnabled ? (
              <input
                className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm outline-none transition focus:border-sky-400"
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Short summary for cards, listings, and SEO previews"
            maxLength={260}
            rows={3}
          ...
          />
          <p className="mt-1 text-xs text-slate-500">{description.length}/260 characters</p>
        </div>

        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-700" htmlFor="scheduledAt">
            Post schedule (optional)
          </label>
          <input
            id="scheduledAt"
            type="datetime-local"
            ...
            className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-base outline-none transition focus:border-sky-400 focus:bg-white"
          />
          <p className="mt-1 text-xs text-slate-500">If published, this post becomes public only at the scheduled time.</p>
        </div>
      </div>
          ...
          {isImageSelected ? (
            <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 px-3 py-3">
              ...
              <div className="ml-2 flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-2 py-1">
                <label htmlFor="image-width-slider" className="text-xs font-semibold text-slate-600">Custom</label>
                <input
                  ...
                  className="w-28"
                />
                <label htmlFor="image-width-input" className="sr-only">
